...
select c.CategoryID ,p.productname,p.productid from products p
inner join categories c on p.categoryID= c.categoryId;

select C.CustomerName,P.product

select OrderID, CustomerID from orders where customerID=34;
select * from employees;
select catagoryname sum(p.price*od.quantity)as ts from categories c.leftjoin product p on p.category=c.category leftjoin orderdetails.od on productid=od.product
group by categoryname;
select * from employees;
select o.orderID,c.customername,e.firstname,e.lastname from orders o join employees e on o.employeeid=e.employeeid join customers c on o.customerid=c.customerid;
select s.suppliername,s.supplierid,count(p.productid) as productcount from suppliers s
left join products p on p.supplierid=s.supplierid group by s.suppliername,s.supplierid; 
